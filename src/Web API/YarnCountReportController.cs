...
using NX_lims_Softlines_Command_System.src.Application.Service.YarnCountContext;
using NX_lims_Softlines_Command_System.src.Domain.Share;

namespace NX_lims_Softlines_Command_System.src.Web_API;

/// <summary>纱支(Yarn Count)报告 API — 生成 docx / 下载</summary>
[ApiController]
[Route("api/[Controller]")]
public class YarnCountReportController : ControllerBase
{
    private readonly IYarnCountReportService _reportService;
    private readonly IWebHostEnvironment _env;

    public YarnCountReportController(IYarnCountReportService reportService, IWebHostEnvironment env)
    {
        _reportService = reportService;
        _env = env;
    }

    /// <summary>生成纱支报告 docx</summary>
    [HttpPost("report")]
    public Result<DocxUrlResponseDto> Generate([FromBody] YarnCountReportRequestDto dto)
    ...
    /// <summary>
    /// 定位报告文件。新报告按月存 wwwroot/DocxModel/SaveDocx/{YarnCount+yyyyMM}/(与生成侧共用
    /// YarnCountReportService.MonthlyFolder), 老报告可能在 SaveDocx/ 根目录, 还要兼容
    ...
    private string? ResolveReportFile(string fileName)
    {
        ...
        string monthly = Path.Combine(root, YarnCountReportService.MonthlyFolder(), fileName);
