...
{
    public class FormulaValidateService
    {
        /// <summary>
        ...
        public Result ValidateStructureAssociation(Formula formula, ParamStructure structure)
        {
            ...
            {
                var contains = formula.ParamSturctureIds != null &&
                    formula.ParamSturctureIds.Any(ps => ps != null && ps.Equals(structure.Id));
                if (!contains)
        ...
        /// <summary>
        /// 原子校验：Formula 是否覆盖 ParamStructure 所需的 condition requirements
