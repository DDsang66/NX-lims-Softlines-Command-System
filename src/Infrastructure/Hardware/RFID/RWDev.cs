// ============================================================================
// UHF RFID 读写器 — 动态库 UHFReader288.dll P/Invoke 包装层
// 来源: Test1 SDK 示例代码 (e:\Test1\rfid-docs\LT-DS8\中文版\开发示例代码\c#\)
// 设备型号: LT-DS8  连接方式: USB 转串口  协议: EPC Gen2 (ISO 18000-6C)
//
// 【本模块被 RfidService 调用，实际用到的方法】（代码中以 "【使用】" 标注）:
//   OpenUSBPort(ComAddr, PortHandle)     — 打开 USB 连接
//   CloseUSBPort(PortHandle)             — 关闭连接
//   Inventory_G2(ComAddr, QValue, ...)   — 单次盘点（扫描标签，返回 EPC 数据）
//   ByteArrayToHexString(byte[])         — 原始字节 → EPC 十六进制字符串
//   HexStringToByteArray(string)         — EPC 字符串 → 原始字节
//
// 其余 100+ 个 DllImport 均为 SDK 完整 API，保留备用（读写锁杀、参数配置、国标/国军标等）
// ============================================================================
using System;
...
namespace NX_lims_Softlines_Command_System.src.Infrastructure.Hardware.RFID
{
    ...
    public static class RWDev
    {
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int ExtSetRegion(ref byte ComAdr, byte flag, byte band,
                                           byte dmaxfre,
                                           byte dminfre,
                                           int frmComPortindex);

        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int ExtGetRegion(ref byte ComAdr, ref byte band,
                                           ref byte dmaxfre,
                                           ref byte dminfre,
                                           int frmComPortindex);
        ...
        // ============================================================
        // 【使用】单次盘点 — EPC Gen2 标签扫描，返回 EPC 数据
        //   QValue=4, Session=1, Scantime=10(1秒) 为常用参数
        // ============================================================
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int SetExtProfile(ref byte ComAdr,
                                             byte Opt,
                                             ref int Profile,
                                             int frmComPortindex);
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int GetEx10Version(ref byte ComAdr,
                                              byte[] Version,
                                              byte[] ex10Type,
                                              int frmComPortindex);

        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int YL_LED_Controll(ref byte ComAdr,
                                              byte[] EPC,
                                              byte ENum,
                                              byte Mode,
                                              byte CarrTime,
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int KLW_LED_Controll(ref byte ComAdr,
                                              byte[] EPC,
                                              byte ENum,
                                              byte CarrTime,
                                              byte[] Password,
        ...
        /// <summary>
        /// 【使用】字节数组 → 十六进制 EPC 字符串（如 "E2003412B0..."）
        /// </summary>
