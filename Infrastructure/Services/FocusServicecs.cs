...
{
    public class FocusService : IBuyerService
    {
        private readonly FocusRepository _repo;
        private readonly FiberContentHelper _helper;

        public FocusService(FocusRepository repo, FiberContentHelper helper)
        {
            _repo = repo;
            _helper = helper;
        }
        ...
        public async Task<object?> ShowParameterAsync([FromBody] RequiredInfoDto infoDto)
        {
            ...
            FocusParameterProvider helper = new FocusParameterProvider(_helper);
            // 生成对应 DTO
            try
            {
                ...
                foreach (var item in items!)
                {
                    ...
                    dtos.Add(FocusParameterMapper.Map(item.itemName!,wetParams ?? new WetParameterIso { ContactItem = item.itemName!, Standard = item.standards }, param!));
                }
                return dtos;
            }
            catch (Exception ex)
