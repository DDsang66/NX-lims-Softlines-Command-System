...
{
    public class LTAGService : IBuyerService
    {
        private readonly LTAGRepository _repo;
        private readonly FiberContentHelper _helper;

        public LTAGService(LTAGRepository repo, FiberContentHelper helper)
        {
            _repo = repo;
            _helper = helper;
        }
        ...
        public async Task<object?> ShowParameterAsync([FromBody] RequiredInfoDto infoDto)
        {
            ...
            LTAGParameterProvider helper = new LTAGParameterProvider(_helper);
            // 生成对应 DTO
            try
            {
                ...
                foreach (var item in items!)
                {
                    ...
                    dtos.Add(LTAGParameterMapper.Map(item.itemName!,wetParams ?? new WetParameterAatcc { ContactItem = item.itemName!, Standard = item.standards }, param!));
                }
                return dtos;
            }
            catch (Exception ex)
