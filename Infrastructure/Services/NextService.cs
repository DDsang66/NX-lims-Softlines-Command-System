...
namespace NX_lims_Softlines_Command_System.Infrastructure.Services
{
    public class NextService : IBuyerService
    {
        ...
        /// <summary>
        /// 根据传入的参数，返回WetParameterIso类型的对象
        /// </summary>
        /// <param name="infoDto"></param>
        /// <returns></returns>
        public async Task<object?> ShowParameterAsync([FromBody] RequiredInfoDto infoDto)
        {
            ...
            try
            {
                ...
                foreach (var item in items!)
                {
                    ...
                    dtos.Add(NextParameterMapper.Map(item.itemName!, wetParams ?? new WetParameterIso { ContactItem = item.itemName!, Standard = item.standards }, param!));
                }
                return dtos;
            }
            catch (Exception ex)
