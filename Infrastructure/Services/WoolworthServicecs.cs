...
{
    public class WoolworthService : IBuyerService
    {
        private readonly WoolworthRepository _repo;
        private readonly FiberContentHelper _helper;

        public WoolworthService(WoolworthRepository repo, FiberContentHelper helper)
        {
            _repo = repo;
            _helper = helper;
        }
        ...
        public async Task<object?> ShowParameterAsync([FromBody] RequiredInfoDto infoDto)
        {
            ...
            WoolworthParameterProvider helper = new WoolworthParameterProvider(_helper);
            // 生成对应 DTO
            try
            {
                ...
                foreach (var item in items!)
                {
                    ...
                    dtos.Add(WoolworthParameterMapper.Map(item.itemName!,wetParams ?? new WetParameterIso { ContactItem = item.itemName!, Standard = item.standards }, param!));
                }
                return dtos;
            }
            catch (Exception ex)
