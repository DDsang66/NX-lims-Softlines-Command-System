...
namespace NX_lims_Softlines_Command_System.Infrastructure.Data.Repositories.BuyerRepos
{
    ...
    //与数据库交互
    public class WoolworthRepository : IRepository
    {
        private readonly LabDbContextSec _db;
        private readonly FiberContentHelper _helper;
        public WoolworthRepository(LabDbContextSec db, FiberContentHelper helper)
        {
            _db = db;
            _helper = helper;
        }
        ...
        {
            // 只处理指定 item 类型
            if (!new[] { "CF to Washing", "DS to Washing","Appearance", "Spirality/Skewing" }
                 .Contains(itemName))
                return default;
            var Param = await _db.WetParameterIsos
                              .FirstOrDefaultAsync(p => p.ContactItem == itemName && p.ReportNumber == input.OrderNumber);
            WoolworthParameterProvider wetParam = new WoolworthParameterProvider(_helper);
            if (Param != null)
            {
