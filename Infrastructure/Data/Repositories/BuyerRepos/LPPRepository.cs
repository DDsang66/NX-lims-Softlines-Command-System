    ...
    public class LPPRepository : IRepository
    {
        ...
        public async Task<List<CheckListDto>?> GetCheckListAsync(dynamic input)
        {
            try
            {
                ...
                var Menu = await _db.Menus.FirstOrDefaultAsync(m => m.MenuName == menuName&&m.ContactBuyer=="Lpp");
        ...
        {
            // 只处理指定 item 类型
            if (!new[] { "CF to Washing", "DS to Washing", "DS to Dry-clean" , "Water Repellency-Spray Test", "Water Resistance-Hydrostatic Pressure", "Spirality/Skewing","CF to Saliva" }
                 .Contains(itemName))
                return default;
