...
namespace NX_lims_Softlines_Command_System.Infrastructure.Providers.Mapper
{
    public class KikParameterMapper
    {
        private static readonly Dictionary<string, Func<WetParameterIso, string, ParamDto>> Mappings = new()
        {
            ...
            ["DS to Washing" ]= (p, param) => new(p.ContactItem!, p.Standard, p.Temperature + "°F", null, null, p.Ballast, p.SpecialCareInstruction, p.DryProcedure, p.WashingProcedure, null, null, null, null),
            ["Appearance"] = (p, param) => new(p.ContactItem!, p.Standard, p.Temperature + "°F", null, null, p.Ballast, p.SpecialCareInstruction, p.DryProcedure, p.WashingProcedure, null, null, null, param),
            ["DS to Dry-clean" ]= (p, param) => new(p.ContactItem!, p.Standard, null, null, null, null, null, null, null, p.Sensitive, null, null, null),
            ["Spirality/Skewing"] = (p, param) => new(p.ContactItem!, p.Standard, p.Temperature + "°F", null, null, p.Ballast, p.SpecialCareInstruction, p.DryProcedure, p.WashingProcedure, null, null, null, null),
            ["Attachment Strength"] = (p, param) => new(p.ContactItem!, p.Standard, p.Temperature + "°C", null, null, p.Ballast, p.SpecialCareInstruction, p.DryProcedure, p.WashingProcedure, null, null, null, null),
